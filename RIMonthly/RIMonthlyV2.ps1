...
$Servers = Import-CSV -Path $serversList | select-object -Property Client,ServerName,BaselinesDir,BackupsDir,DataDir,MonthlyDir
...
ForEach ($server in $Servers)
{
    ...
    Write-Log "The Backup directory is: $BackupsDir"
    ...
    $argsBCSS = "@$bcSnap /closescript $BackupsDir $bcssCurDate $BaselinesDir"
    ...
    If ($prevBCSSFileExists -eq $True)
    {
        ...
        $argsBCSS = "@$bcCopy /closescript $BackupsDir $prevBCSSFile $MonthlyDir $bcssPrevDate"
        Start-Process -FilePath $bcApp -ArgumentList $argsBCSS -wait
