...
Param ([Parameter(Mandatory=$true)] [boolean] $ExecuteFound=$False
      ,[Parameter(Mandatory=$true)] [boolean] $ExecuteNew=$False)
...
if ($lContinue)
{
   ...
   foreach ($row in $oSource)
   {
      ...
      {
         $logMsg = "Error - found Position in CSV file at line $nCount, that does not match known positions in CSV file: $cPositionOUFile"
         logThis "$logMsg"
         Write-Host $logMsg -ForegroundColor Red
         $lContinue = $False
      }
         ...
         if ($lExecuteNew)
         {
            ...
            switch ($cUserType.ToLower())
            {
               ...
               "email"
               {
                  ...
                  Try
                  {
                     $oCASMailbox | Set-CASMailbox -ActiveSyncEnabled $False -EWSEnabled $False -OWAForDevicesEnabled $False -OWAMailboxPolicy "ExternalUsers"
                  }
                  Catch
                  {
